...
void main(int argc, char **argv) {
	...
	while (ro > 0.01) {
		...
		// Jprev = J
	   /** __m128 m1, m2;
	    int loop = howManyTimeSteps/4;
	    __m128** pSrc1 = (__m128**) Jactual;
	    for (i = 0; i < loop; i++)
			for (j = 0; j < startStateDivision * startStateDivision; j++)
				_mm_store_ps(*Jprev,** pSrc1);
...
float norm(float **matrix, float **odejmowana, int y, int x) {
    ...
    __m128 m1, m2;
    __m128** pSrc1 = (__m128**) matrix;
    __m128** pSrc2 = (__m128**) odejmowana;
	...
	__m128** helper = (__m128**) a;

	//odejmowanie
	for (i = 0; i < x; i++)
		for (j = 0; j < nLoop; j++) {
			
			m1 = _mm_sub_ps(**pSrc1,**pSrc2);
			m2 = _mm_mul_ps(m1,m1);
			_mm_store_ss(&sum1,m2);
			sum += sum1;
		}
